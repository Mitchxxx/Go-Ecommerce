...
func main() {
	...
	// Create SQS Subcriber
	logger := watermill.NewStdLogger(false, false)
	subcriber, err := sqs.NewSubscriber(sqs.SubscriberConfig{
		AWSConfig: awsConfig,
	}, logger)
	...
	messages, err := subcriber.Subscribe(ctx, cfg.AWS.EventQueueName)
	if err != nil {
		log.Fatalf("Failed to subcribe to queue: %v", err)
	}
	...
	for {
		...
		case <-sigChan:
			...
			err := subcriber.Close()
			if err != nil {
				log.Fatalf("unable to close Subscriber %v", err)
			}
			return
		}
	}
